...
SEMÁFORO — DETECCIÓN DE AMARILLO ROBUSTA:
  • Tres rangos HSV cubren H=10–38 para capturar amarillo puro,
    amarillo-naranja (reflexión lateral) y amarillo-verde (esquinas).
  • El verde excluye explícitamente los píxeles ya reclamados por
    el amarillo (bitwise_not), eliminando falsos positivos.
═══════════════════════════════════════════════════════════════
"""
...
SIGN_MAX_DIST_CM  = 0.25   # ← AJUSTAR tras calibrar (ver instrucciones arriba)
...
class VisionNode(Node):
    ...
    # ─────────────────────────────────────────────────────────
    # SEMÁFORO — detección robusta de amarillo
    #
    # Problema real: semáforo circular con brillo → el centro
    # queda casi blanco (S baja, V muy alta) y los bordes tienen
    # H variable (10-38).  Necesitamos S desde 50 para no perder
    # el núcleo brillante, y V desde 180 para exigir que sea luz.
    #
    # Estrategia de 3 rangos HSV para amarillo:
    #   yel_a H=15-30  núcleo puro/brillante   S≥50  V≥180
    #   yel_b H=30-38  borde verdoso            S≥80  V≥160
    #   yel_c H=10-15  borde anaranjado         S≥80  V≥180
    #
    # El verde exige S≥150 (luz verde real es muy saturada) y
    # excluye explícitamente píxeles ya reclamados por amarillo.
    # ─────────────────────────────────────────────────────────
    ...
    def detect_traffic_light(self, frame: np.ndarray) -> str:
        ...
        # ── AMARILLO ROBUSTO — semáforo circular con brillo ───
        # S baja desde 50: el centro sobreexpuesto pierde saturación
        # V alta desde 180: exigimos que sea luz, no superficie amarilla
        # yel_a: núcleo puro/brillante  H=15-30  S≥50  V≥180
        # yel_b: borde verdoso          H=30-38  S≥80  V≥160
        # yel_c: borde anaranjado       H=10-15  S≥80  V≥180
        yel_a = cv2.inRange(hsv, (15,  50, 180), (30, 255, 255))
        yel_b = cv2.inRange(hsv, (30,  80, 160), (38, 255, 255))
        yel_c = cv2.inRange(hsv, (10,  80, 180), (15, 255, 255))
        yellow_raw = yel_a | yel_b | yel_c
        # Dilatar para unir el núcleo brillante con los bordes
        yellow_raw = cv2.dilate(yellow_raw, k3, iterations=2)
        yellow     = cv2.morphologyEx(yellow_raw, cv2.MORPH_OPEN, k3)

        # ── VERDE — S≥150 (luz verde real muy saturada) ───────
        # Excluye píxeles ya reclamados por amarillo.
        green_raw = cv2.inRange(hsv, (40, 150, 100), (85, 255, 255))
        green_raw = cv2.bitwise_and(green_raw, cv2.bitwise_not(yellow_raw))
        green     = cv2.morphologyEx(green_raw, cv2.MORPH_OPEN, k3)
    ...
    def process_frame(self):
        ...
        # ── Label de estado de señal activa para el HUD ──────
        # SLOWING muestra el nombre real de la señal + 50% en speed
        if self._sign_action_state == "SLOWING":
            slbl = f" [{sign_activa} 50%]"
        else:
            slbl = {"IDLE": "", "TURNING": f" [{self._sign_turn_dir}]",
                    "REINCORP": " [REINCORP]", "STOPPING": " [STOP]"
                    }.get(self._sign_action_state, "")

        # Speed display: siempre 50% mientras SLOWING, aunque el robot no frene
        speed_display = (SLOW_SPEED_RATIO
                         if self._sign_action_state == "SLOWING"
                         else self._speed_multiplier)
        ...
        cv2.putText(frame, f"SPEED:{speed_display:.0%}",
